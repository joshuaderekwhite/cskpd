...
The `CSKPD` class is initialized with several key parameters:
...
- **`fit()`**: Fits the model using your data.
- **`custom_cross_validate()`**: Performs custom cross-validation with specified scorers (e.g., MSE, AUC).

---

## 3. Example Implementation

Here's an example of how to create and use a `CSKPD` model:
...
The `custom_cross_validate` method allows you to evaluate the model's performance using custom scorers:
...
### Example Output from `custom_cross_validate`:
```python
{
    'params': {
        'R': 3,
