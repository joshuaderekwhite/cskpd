...
\State \textbf{Initialization:} $\hat{A}^{(0)}$ is taken as the top-R left singular vectors of $\sum_{i=1}^n X_i y_i$; with $\hat{X}_i = R(X_i)$ for matrix image or $\hat{X}_i = R(X_i)$ for tensor image, $\zeta^{(0)}=\vec{0}$, $z^{(0)}=\vec{0}$ and $(t) = 1, \ldots, T$.
\For{$t \in \{0, 1, 2, \ldots, T-1\}$}
    ...
    \State $\hat{A}^{(t+1)} \gets \min_A \frac{1}{2n} \sum_{i=1}^n (y_i - z_i^{(t)} - [\text{vec}(A)]^\top \text{vec}(\hat{X}_i^\top \hat{B}^{(t+1)})^2 + \Sigma_{r=1}^{R} \lambda_{A_j} \|\text{vec}(A_r)\|_1$
